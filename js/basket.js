...
$(document).ready(function () {
	...
	const $basketSelectAll = $('#select-all'),
		$basketAgreement = $('[data-basket-agreement]'),
		$basketDalivery = $('.tab-toggler input[name="delivery"]'),
		$basketDaliveryInputs = $('[data-tab-content="delivery"]').find('input, textarea'),
		$basketDaliveryVal = $('[data-basket-delivery]'),
		$basketSubmit = $('[data-basket-submit]'),
		$basketBtnAll = $('[data-basket-all]'),
		$basketItemsClear = $('[data-basket-clear]'),
		$basketBtnUnavailable = $('[data-basket-unavailable]'),
		$basketDeleted = $('[data-basket-deleted]'),
		$basketSelectedCount = $('.basket [data-basket-selected-count]'),
		$basketCount = $('.basket [data-basket-count]'),
		$basketUnavailableCount = $('[data-basket-unavailable-count]'),
		$basketTotal = $('.basket [data-basket-total]'),
		$basketItemsWrap = $('.basket-items'),
		$basketEmptyBlock = $(`
	...
	$basketDaliveryInputs.prop('disabled', true).trigger('change');
	...
	$basketDalivery.on('change', function () {
		...
		if (isPickup) {
			$basketDaliveryVal.html('Бесплатно')
			$basketDaliveryInputs.prop('disabled', true).trigger('change')
		}
		else {
			$basketDaliveryVal.html(
				`От 1000 ₽ <div class="info">
					<svg class="ic ic-md"><use href="img/ic.svg#info"></use></svg>
					<div class="info__text">Стоимость и срок доставки уточнит менеджер при подтверждении заказа</div>
				</div>`
			);
			$basketDaliveryInputs.prop('disabled', false).trigger('change')
		}
		totalCounter();
	});
	...
	function basketAgreement() {
		const allChecked = $basketAgreement.length > 0 &&
			$basketAgreement.filter(':checked').length === $basketAgreement.length;
	...
	$('[data-basket-remove]').on('click', function () {
		...
		const deletedBlock = $(`
			<div class="basket-deleted">
				...
				<button type="button" class="basket-btn c-danger basket-restore">Восстановить</button>
				<button type="button" class="basket-deleted__dell">
